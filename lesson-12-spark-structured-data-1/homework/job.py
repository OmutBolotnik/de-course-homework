...
def clean(events: DataFrame) -> DataFrame:
    return (
        ...
        .filter(F.col("public").isNotNull() & (F.col("public") == True))
        .filter(
...
def with_derived(events: DataFrame) -> DataFrame:
    return events.withColumns({
        ...
        "is_bot": F.coalesce(F.col("actor_login").endswith(BOT_SUFFIX), F.lit(False)),
...
def top_repos_per_type(events: DataFrame, n: int) -> DataFrame:
    counted = events.groupBy("event_type", "repo_name").agg(
...
def enrich_top_repos(top_repos: DataFrame, owners: DataFrame) -> DataFrame:
    ...
    joined = with_owner.join(F.broadcast(owners), "repo_owner", "left")
    return joined.select(
        ...
        F.when(
            F.coalesce(F.col("owner_events"), F.lit(0)) == 0, F.lit(None)
        ).otherwise(
